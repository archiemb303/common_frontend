...
from common.registration.api.signup_mobileotp_verification.validations_signup_mobileotp_verify import \
    validation_signupmobileotpverify
...
class SignUpMobileOtpVerifyAPI(APIView):
    ...
    def post(self, request):
        """Post Function to authenticate and send sms to new as well as existing user for registration ."""
        ...
        try:
            # Fetch row from mobile verification table for input phone number
            sql = sql_exec("fetch_mobileverification_row",
                           [input_json['phone_number']])[0]
            if sql['otp'] == input_json['otp']:
                # Fetch user profile row from mobile verification table for input phone number.
                profile_data = fetch_profilefromphonenumber(input_json)
    ...
    def insert_usersessions(self, request):
        ...
        try:
            session_var = dict(zip(['profile_id', 'session_key', 'login_source', 'login_type', 'added_by',
                                   ...
                                   [input_json['profile_id'], get_random_string(length=32), 'mobile', 'native',
    ...
    def check_profile_fields(self, request):
        ...
        try:
            ...
            output_json = dict(zip(['Status', 'Message', 'Payload'],
                                   ['Failure', f"Error occured during checking for profile fields table."
    ...
    def update_userprofile(self, request, profile):
        ...
        try:
            if profile['profile_completion_status'] == 2:
                update_params = dict(zip(['first_name', 'last_name', 'sex', 'date_of_birth', 'orientation', 'city_id',
                                          'dp_flag_id', 'profile_completion_status_id'], [input_json['first_name'],
                                                                                          ...
                                                                                          input_json['city_id'],
                                                                                          input_json['dp_id'], 1]))
            if profile['profile_completion_status'] == 1:
                update_params = dict(zip(['first_name', 'last_name', 'sex', 'date_of_birth', 'orientation', 'city_id',
                                          'dp_flag_id', 'profile_completion_status_id'],
                                         [input_json['first_name'], input_json['last_name'],
                                          input_json['sex'], input_json['date_of_birth'], input_json['orientation'],
                                          input_json['city_id'], input_json['dp_id'], 2]))
            # update_params = dict(zip(['first_name', 'last_name', 'sex', 'date_of_birth', 'orientation', 'city_id', 'dp_flag_id', 'profile_completion_status'],
            #                          [input_json['first_name'], input_json['last_name'], input_json['sex'], input_json['date_of_birth'],
            #                           input_json['orientation'], input_json['city_id'], input_json['dp_id'], 1]))
            ...
            output_json = dict(zip(['Status', 'Message', 'Payload'],
                                   ['Failure', f"Error encountered during updation in user profile table. "
