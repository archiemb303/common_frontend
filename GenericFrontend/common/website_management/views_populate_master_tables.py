""" This module is used to populate all the master tables that belong to WebsiteManagement app"""

from common.utilities.populate_all_master_tables.populate_master_tables_lib import populate_master_data
from common.website_management.models import WebsiteLaunchInfoStatus, WebsiteDowntimeStatus
from common.website_management.serializers import WebsiteLaunchInfoStatusSerializer, WebsiteDowntimeStatusSerializer


def populate_website_management_master_tables(table_population_output_list):
    """
    This function populates all the master table data of the app named website_management
    ...
    try:
        ...
        table_population_output_list = populate_website_launch_info_status(table_population_output_list)
        table_population_output_list = populate_website_downtime_status(table_population_output_list)
        return table_population_output_list

    except Exception as ex:
        output_json = dict(zip(['Status', 'Message', 'Payload'],
                               ["Failure", f"One or more master tables of website_management app could not be populated."
                                           f"Exception encountered: {ex}", table_population_output_list]))
        return output_json


def populate_website_launch_info_status(table_population_output_list):
    """
    This function populates the master table named WebsiteLaunchInfoStatus in the app named website_management
    :return:
    """
    try:
        ...
        model_name = "WebsiteLaunchInfoStatus"
        model_instance = WebsiteLaunchInfoStatus
        model_serializer_instance = WebsiteLaunchInfoStatusSerializer
...
def populate_website_downtime_status(table_population_output_list):
    """
    This function populates the master table named WebsiteDowntimeStatus in the app named website_management
    :return:
    """
    try:
        # defining all the master table rows. This is the final data that should be there in the master table.
        ...
        # define the below mentioned parameters for this model
        primary_key_var = "status_id"
        model_name = "WebsiteDowntimeStatus"
        model_instance = WebsiteDowntimeStatus
        model_serializer_instance = WebsiteDowntimeStatusSerializer
